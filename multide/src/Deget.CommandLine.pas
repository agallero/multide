...
procedure CloseConsoleProcessGracefully(PID: DWORD; var Closing: boolean);
begin
  if Closing then exit;
  Closing := true;
  AttachConsole(PID);
  SetConsoleCtrlHandler(nil, True); // disable self handling
...
begin
  ...
  try
    ...
    Handle := CreateProcess(nil, PChar(LocalCommandLine), nil, nil, True,
    NORMAL_PRIORITY_CLASS or CREATE_UNICODE_ENVIRONMENT or CREATE_NEW_PROCESS_GROUP,     //CREATE_NEW_PROCESS_GROUP is needed so ctrl-c is not passed to this proceess.
    ...
    try
      ...
      repeat
        if (Assigned(IsCanceledFunc) and IsCanceledFunc()) then
        begin
          CloseConsoleProcessGracefully(PI.dwProcessId, Closing);
        end;
